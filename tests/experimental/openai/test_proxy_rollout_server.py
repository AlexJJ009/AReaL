...
async def test_prm_export_error_policy_preserves_original_branches(
    monkeypatch, policy, fail
):
    visited = []
    ...
    class Scorer(BaseScorer):
        ...
        async def evaluate(self, interaction, ctx):
            branch = next(
                m["content"] for m in ctx["messages"] if m.get("role") == "tool"
            )
            visited.append(branch)
            if fail and branch == "branch-b":
                raise RuntimeError("second branch failed")
            return 1.0

    root = _prm_interaction("root", [1], [2], [{"role": "user", "content": "task"}])
    leaves = {
        f"leaf-{name}": _prm_interaction(
            f"leaf-{name}",
            [1, 2, 3],
            [4],
            [{"role": "tool", "content": f"branch-{name}"}],
            parent=root,
        )
        for name in ["a", "b"]
    }
    originals = [root, *leaves.values()]
    for interaction in originals:
        interaction.reward = 0.75
        interaction.token_rewards = torch.tensor([0.125])
        interaction._cache = {
            "rewards": torch.tensor([0.75]),
            "sentinel": torch.tensor([2, 3]),
        }
    before = [{k: v.clone() for k, v in item._cache.items()} for item in originals]
    session = SessionData(session_id="policy-test")
    session.finish()
    monkeypatch.setattr(session, "export_interactions", lambda **kwargs: leaves)
    srv._session_cache[session.session_id] = session
    srv._session_to_api_key[session.session_id] = "policy-key"
    srv._api_key_to_session["policy-key"] = session.session_id
    monkeypatch.setattr(
        srv,
        "_prm_runner",
        PRMRunner(PRMConfig(error_policy=policy, scorers=[Scorer()])),
    )
    captured = {}

    def serialize(interactions, **kwargs):
        captured.update(interactions)
        return {}

    monkeypatch.setattr(srv, "serialize_interactions", serialize)
    stats_tracker.export_all(reset=True)
    await srv.export_trajectories(
        srv.ExportTrajectoriesRequest(session_id=session.session_id, style="concat")
    )
    assert "branch-a" in visited and "branch-b" in visited
    assert (not captured) == (fail and policy == "reject")
    for original, tensors in zip(originals, before):
        assert original.reward == 0.75
        torch.testing.assert_close(
            original.token_rewards, torch.tensor([0.125]), rtol=0, atol=0
        )
        for key, value in tensors.items():
            torch.testing.assert_close(original._cache[key], value, rtol=0, atol=0)
    if fail and policy == "keep_original":
        assert all(captured[key] is leaf for key, leaf in leaves.items())
        assert all(leaf.parent is root for leaf in captured.values())
    elif not fail:
        assert all(captured[key] is not leaf for key, leaf in leaves.items())
        for leaf in captured.values():
            torch.testing.assert_close(leaf.token_rewards, torch.tensor([1.125]))
    metrics = stats_tracker.export_all(reset=True)
    if fail:
        assert "rollout/prm_turn_reward/policy_test" not in metrics
        assert "rollout/prm_trajectory_reward/policy_test" not in metrics
    assert metrics["rollout/prm_fallback"] == float(fail and policy == "keep_original")
    assert session.session_id not in srv._session_cache
    assert "policy-key" not in srv._api_key_to_session


def test_prm_error_policy_rejects_unknown_value():
    assert PRMConfig().error_policy == "reject"
    with pytest.raises(ValueError, match="error_policy"):
        PRMConfig(error_policy="invalid")


@pytest.mark.asyncio
async def test_prm_keep_original_does_not_swallow_cancellation(monkeypatch):
    import asyncio
    from unittest.mock import AsyncMock

    session = SessionData(session_id="cancel-policy")
    session.finish()
    monkeypatch.setattr(
        session,
        "export_interactions",
        lambda **kwargs: {"a": _prm_interaction("a", [1], [2], [])},
    )
    srv._session_cache[session.session_id] = session
    monkeypatch.setattr(
        srv, "_prm_runner", PRMRunner(PRMConfig(error_policy="keep_original"))
    )
    monkeypatch.setattr(
        srv, "_score_prm_branches", AsyncMock(side_effect=asyncio.CancelledError())
    )
